...
def create_app():
    ...
    def before_request():
        excluded_routes = ['views.favicon','views.registeracc' , 'views.monitor','views.login', 'views.login2', 'shortlinks.tools', 'vdo.commandslist', 'shortlinks.youtube', 'vdo.cmdcommand' , 'vdo.storjflask2']
        if request.endpoint and request.endpoint not in excluded_routes and not request.path.startswith('/static/') and not request.path.startswith('/send_email') and not request.path.startswith('/admin'):
