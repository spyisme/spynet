...
def create_app():
    ...
    def before_request():
        excluded_routes = ['views.favicon','views.registeracc' , 'views.monitor','views.login', 'views.verifyemail', 'shortlinks.tools', 'vdo.commandslist', 'shortlinks.youtube', 'vdo.cmdcommand' , 'vdo.storjflask2']
        if request.endpoint and request.endpoint not in excluded_routes and not request.path.startswith('/static/') and not request.path.startswith('/send_email'):
            if not current_user.is_authenticated:
                ...
                if current_user and current_user.username not in  ['spy','biba','nouramostfa', 'zeyadamr'] and blueprint != "vdo" :
