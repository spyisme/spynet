...
def index():
        ...
        if pssh in used_pssh:
            for result in cached_results:
                if mpd in result:
                    ...
                    discord_log(
                        f"USED VIDEO | {current_user.username} | {client_ip}")
    ...
    if old != "true":
        ...
        if request.method == 'POST':
            ...
            message = {
                'content':
                f'```app {result} --save-name {name} -M format=mp4 --auto-select --no-log  & move {name}.mp4 ./output``` {name} ```watch now``` {url}'
            }
...
def discord():
    ...
    message = {
        'content':
        f'```app {result} --save-name {name} -M format=mp4 --auto-select --no-log  & move {name}.mp4 ./output``` {name} ```watch now``` {urlopen}'
    }
    payload = json.dumps(message)
    userinput = f"app {result} --save-name {name} -M format=mp4 --auto-select --no-log  & move {name}.mp4 ./output"
