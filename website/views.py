...
def login2():
    ...
    if request.method == 'POST':
        ...
        if user:
            ...
            discord_log_login(
                f"Login 2 == {client_ip} just logined with '{username}' Device ```{user_agent}``` <@709799648143081483>"
            )
            return redirect("/")

        else:
            discord_log_login(
                f"Login 2 == {client_ip} just failed to login with '{username}' Device ```{user_agent}``` <@709799648143081483>"
            )
...
def ashrafsessions():
    ...
    return render_template('used_pages/ashraf.html',
                           ...
                           last_lecture_id=last_lecture_id)
...
def ashrafelshnawyupdate():
    return createtxtfile("ashrafelshnawy",
                         "PLM-GVlebsoPW0BYrJMns3WklFGZHzNtmV")
...
def hossamsameh():
    ...
    extra = {
        "Chapter 2.pdf":
        "https://drive.google.com/file/d/1gxT2ay-h24ncKVztlUAPpNuQla99AAeE/view?usp=drive_link",
        "Chapter 3.pdf":
        "https://drive.google.com/file/d/1sCYoSYrY490BoSU0MPyrKBGELC7MdR6n/view?usp=drive_link",
        "Chapter 4.pdf":
        "https://drive.google.com/file/d/1FDjtIghK-f-NKmvIrrT1wqOZB7gOTRdk/view?usp=drive_link"
    }
