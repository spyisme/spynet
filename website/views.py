...
def login():
  ...
  if client_ip in blacklist_ips:
    return jsonify(message=f"Please disable vpn/proxy. {client_ip}"), 403
  ...
  if client_ip not in whitelist_ips:
    ...
    if 'country' in data:
      ...
      if country_code != 'EG':
        ...
        return jsonify(message=f"Please disable vpn/proxy. {data}"), 403
    else:
      blacklist_ips.add(client_ip)
      return jsonify(
          message=f"Unable to determine the country. Login failed. {data}"
      ), 403
...
def login2():
  ...
  if request.method == 'POST':
    ...
    if user:
      login_user(user)
      discord_log(
          f"{client_ip} - {username}  just logged in Device ```{user_agent}```  <@709799648143081483>"
      )
      session.permanent = True
      return redirect(url_for('views.home'))
    else:
...
def home():
  ...
  lines = [
      "chemistry", "english", "maths", "arabic", "german", "physics",
      "biology", "geology"
  ]
...
def nawarupdate(custom_url):
  info = load_nawar_info()
  course_key = next(
      (name for name, info in info.items() if info['url'] == f"/{custom_url}"),
      None)
...
def nawarvids(custom_url):
  ...
  with open(f"website/playlists/nawar{course_name}.txt", 'r',
            encoding='utf-8') as file:
    content = file.read()
    videos = ast.literal_eval(content)
...
def updateashraf():
  ...
  response = requests.post('https://api.csacademyzone.com/lectures',
                           headers=headers,
                           json=json_data)
  data = response.json()
  filtered_lectures = []
  ...
  for lecture in data['lectures']:
    filtered_lecture = {"id": lecture["id"], "title": lecture["title"]}
  ...
  if request.method == 'POST':
    try:
      ...
      if response.status_code == 401:
        ...
        return jsonify({
            "error":
            f"Failed to get OTP. Status code: {response.status_code}"
        }), 500
...
def ashrafelshnawy():
  ...
  with open("website/playlists/ashrafelshnawy.txt", 'r',
            encoding='utf-8') as file:
    content = file.read()
    videos = ast.literal_eval(content)
  return render_template('used_pages/videopage.html',
...
def nasser():
  ...
  teacher_links = {
      f"Nasser-El-Batal {course}": (f"/nasser{nasser_info[course]['url']}",
...
def mohamedsalah():
  ...
  with open("website/playlists/mohamedsalah.txt", 'r',
            encoding='utf-8') as file:
    content = file.read()
    videos = ast.literal_eval(content)
  return render_template('used_pages/videopage.html',
...
def samehvids(custom_url):
  ...
  teachername = course_name
  playlist_id = course_info["id"]
  with open(f"website/playlists/sameh{course_name}.txt", 'r',
            encoding='utf-8') as file:
    content = file.read()
    videos = ast.literal_eval(content)
...
def daifvids(custom_url):
  ...
  teachername = course_name
  playlist_id = course_info["id"]
  with open(f"website/playlists/daif{course_name}.txt", 'r',
            encoding='utf-8') as file:
    content = file.read()
    videos = ast.literal_eval(content)
